  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        const failCandidates = await db.pool.query(
        ...
        for (const row of failCandidates.rows) {
          ...
          try {
            ...
            const lastFailed = await client.query(
              `select id, node_id, node_type, error, finished_at
               from node_runs
            ...
            await client.query(
              ...
              [tenantId, runId, JSON.stringify(errPayload)],
            );
        ...
        const advCandidates = await db.pool.query(
          `select wr.id as run_id, wr.workflow_version_id
           from workflow_runs wr
          where wr.tenant_id = $1
            and wr.status = 'RUNNING'::workflow_run_status
            and exists (
        ...
        for (const row of advCandidates.rows) {
          const runId = row.run_id as string;
          const workflowVersionId = row.workflow_version_id as string;
          ...
          try {
            ...
            const fromNodeId = last.rows[0].node_id as string;
            const edge = edges.find((e: any) => e?.from === fromNodeId);
        ...
        const readyDelay = await db.pool.query(
          `select nr.id as node_run_id, nr.workflow_run_id, nr.node_id,
        ...
        for (const row of readyDelay.rows) {
          const nodeRunId = row.node_run_id as string;
          const runId = row.workflow_run_id as string;
          ...
          try {
            ...
            const nrLock = await client.query(
              `select id, queued_at
               from node_runs
              where tenant_id = $1 and id = $2
            ...
            const queuedAt = nrLock.rows[0].queued_at as Date | null;
            ...
            await client.query(
             ...
             values ($1, $2, 'info', 'delay completed', $3::jsonb)`,
              [tenantId, nodeRunId, JSON.stringify({ seconds })],
            );

            // Find next node via first matching edge { from, to }
            const edge = edges.find((e: any) => e?.from === nodeId);
            const nextNodeId = edge?.to;
        ...
        for (const row of runs.rows) {
          ...
          try {
            ...
            const existing = await client.query(
              `select 1 from node_runs where tenant_id = $1 and workflow_run_id = $2 limit 1`,
              [tenantId, runId],
            );
        ...
        return reply.code(200).send({
          ...
          delay_checked:
            typeof readyDelay !== "undefined" ? readyDelay.rowCount : 0,
