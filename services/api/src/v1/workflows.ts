...
export async function registerWorkflowsRoutes(app: FastifyInstance) {
  ...
  app.get("/", async (req, reply) => {
    try {
      ...
      const r = await db.pool.query(
        "select id, tenant_id, name, draft_updated_at, created_at, updated_at from workflows where tenant_id = $1 order by created_at desc limit 100",
        [tenantId],
      );
      return { items: r.rows };
    } catch (e) {
    ...
    try {
      ...
      const r = await db.pool.query(
        `insert into workflows (tenant_id, name, draft_graph)
         values ($1, $2, '{}'::jsonb)
         returning id, tenant_id, name, draft_graph, draft_updated_at, created_at, updated_at`,
        [tenantId, name],
      );
  ...
  app.get("/:workflowId", async (req, reply) => {
    try {
      ...
      const r = await db.pool.query(
        "select id, tenant_id, name, draft_graph, draft_updated_at, created_at, updated_at from workflows where tenant_id = $1 and id = $2",
        [tenantId, workflowId],
      );
  ...
  app.get("/:workflowId/versions", async (req, reply) => {
    try {
      ...
      const workflowId = (req.params as any).workflowId as string;
      const r = await db.pool.query(
        `select id, tenant_id, workflow_id, version, graph, published_at, created_at
  ...
  app.get("/:workflowId/versions/:versionId", async (req, reply) => {
    try {
      const tenantId = getTenantIdFromHeaders(req.headers);
      const workflowId = (req.params as any).workflowId as string;
      const versionId = (req.params as any).versionId as string;
      const r = await db.pool.query(
        `select id, tenant_id, workflow_id, version, graph, published_at, created_at
           from workflow_versions
  ...
  app.patch(
    ...
    async (req, reply) => {
      try {
        ...
        const sets: string[] = [];
        const values: any[] = [tenantId, workflowId];
      ...
      try {
        ...
        try {
          ...
          const wf = await client.query(
            "select id, tenant_id, name, draft_graph from workflows where tenant_id = $1 and id = $2 for update",
            [tenantId, workflowId],
          );
          if (wf.rowCount === 0) {
          ...
          const graph =
            parsed.data.draft_graph !== undefined
          ...
          const created = await client.query(
           ...
           returning id, tenant_id, workflow_id, version, published_at, created_at`,
  ...
  app.post(
    "/:workflowId/versions/:versionId/runs",
    { config: { requiredRole: "editor" } },
    async (req, reply) => {
      try {
        const tenantId = getTenantIdFromHeaders(req.headers);
        ...
        const r = await db.pool.query(
          ...
          [tenantId, versionId, JSON.stringify(parsed.data.input ?? {})],
        );
