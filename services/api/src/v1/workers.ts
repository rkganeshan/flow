      ...
      try {
        ...
        try {
          ...
          for (const r of pick.rows) {
            const nodeRunId = r.id as string;
            const upd = await client.query(
              ...
              returning id, workflow_run_id, node_id, node_type, status, started_at, queued_at`,
              [tenantId, nodeRunId],
            );

            if (upd.rowCount === 1) claimed.push(upd.rows[0]);
          }
  ...
  app.post(
    ...
    async (req, reply) => {
      try {
        ...
        const env = (app as any).env as any;
        ...
        try {
          ...
          const nr = await client.query(
            `select id, workflow_run_id, node_id, node_type, status
             from node_runs
            where tenant_id = $1 and id = $2
            for update`,
            [tenantId, nodeRunId],
              ...
              const done = await client.query(
                `update node_runs
                  set status = 'FAILED_FINAL'::node_run_status,
                      ...
                      updated_at = now()
                where tenant_id = $1 and id = $2
                returning id, workflow_run_id, node_id, node_type, status, started_at, finished_at, error`,
                [tenantId, nodeRunId, JSON.stringify(err)],
              );
...
function parseAllowedHosts(v: unknown): Set<string> | null {
...
function isHostAllowed(host: string, allowed: Set<string> | null): boolean {
