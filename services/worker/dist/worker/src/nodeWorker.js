...
import { QUEUE_DLQ, QUEUE_ENGINE_ADVANCE, QUEUE_NODE_EXECUTE, } from "./queues.js";
...
function isHostAllowed(hostname, allow) {
...
function createNotifyProviders(deps) {
    return {
        email: async ({ ctx, client }) => {
            ...
            const text = resolvedText !== undefined
...
function isRetryableNotifyError(e) {
...
async function buildRunContext(client, tenantId, runId) {
    const run = await client.query(`select input
       from workflow_runs
      where tenant_id = $1 and id = $2`, [tenantId, runId]);
    const input = run.rowCount === 1 ? (run.rows[0].input ?? {}) : {};
    const outs = await client.query(`select node_id, output, outcome
       from node_runs
      where tenant_id = $1 and workflow_run_id = $2 and status = 'SUCCEEDED'::node_run_status`, [tenantId, runId]);
    const nodes = {};
...
function resolveContextValue(args) {
    return getByPath(args.runContext, args.path);
}
function createNodeHandlers(deps) {
    ...
    return {
        ...
        notify: async ({ ctx, client }) => {
            ...
            try {
                ...
                const err = new Error(`notify provider failed (${provider})${retryable ? " [retryable]" : ""}: ${String(e?.message ?? e)}`);
                throw err;
            }
        },
    };
}
export async function registerNodeWorker(deps) {
    ...
    async function enqueueDlq(args) {
        try {
            await dlqQueue.add("failed_node_run", {
    ...
    const nodeExecuteWorker = new Worker(QUEUE_NODE_EXECUTE, async (job) => {
        ...
        try {
            ...
            // Lock node_run.
            const nr = await client.query(`select id, tenant_id, workflow_run_id, node_id, node_type, status,
            ...
            if (row.status === "SUCCEEDED" || row.status === "FAILED_FINAL") {
                ...
                await engineAdvanceQueue.add("advance", { node_run_id: nodeRunId }, {
            ...
            const run = await client.query(`select workflow_version_id, status
             from workflow_runs
            ...
            if (run.rows[0].status !== "RUNNING") {
                ...
                set status = 'READY'::node_run_status,
                    updated_at = now()
              where id = $1 and status = 'IN_PROGRESS'::node_run_status`, [nodeRunId]);
                await client.query("commit");
            ...
            const runContext = await buildRunContext(client, tenantId, String(row.workflow_run_id));
            const ctx = {
                tenantId,
            ...
            const failWithRetryPolicy = async (errObj) => {
                ...
                if (nextAttempt < maxAttempts) {
                  ...
                  set status = 'FAILED_RETRYABLE'::node_run_status,
                      ...
                      error = $3::jsonb,
                      updated_at = now()
                where id = $1`, [nodeRunId, nextAttempt, JSON.stringify(errObj)]);
                    await client.query(`insert into node_run_logs (tenant_id, node_run_id, level, message, data)
               values ($1, $2, 'warn', 'node failed (retry scheduled)', $3::jsonb)`, [
                    ...
                    await client.query("commit");
                    await nodeExecuteQueue.add("execute", { node_run_id: nodeRunId }, {
            ...
            if (row.node_type === "delay") {
                ...
                const delayMs = Number.isFinite(seconds)
                    ...
                    : 0;
                const queuedAt = row.queued_at ?? null;
                ...
                set status = 'SUCCEEDED'::node_run_status,
                    ...
                    updated_at = now()
              where id = $1
              returning id, status`, [nodeRunId, JSON.stringify(output)]);
                await client.query(`insert into node_run_logs (tenant_id, node_run_id, level, message, data)
             values ($1, $2, 'info', 'delay completed', $3::jsonb)`, [tenantId, nodeRunId, JSON.stringify(output)]);
                await client.query("commit");
                await engineAdvanceQueue.add("advance", { node_run_id: nodeRunId }, {
                    jobId: `advance-${nodeRunId}`,
                    removeOnComplete: 1000,
            ...
            if (row.node_type === "http_request") {
                ...
                const headersContextPath = String(cfg.headers_context_path ?? "").trim();
                ...
                const dynamicBody = bodyContextPath
                    ...
                    : undefined;
                const bodyValue = dynamicBody !== undefined
                    ? dynamicBody
                    : cfg.body
                        ? cfg.body
                        : undefined;
                ...
                set status = 'SUCCEEDED'::node_run_status,
                    ...
                    output = $2::jsonb,
                    updated_at = now()
              where id = $1`, [nodeRunId, JSON.stringify(output)]);
                await client.query(`insert into node_run_logs (tenant_id, node_run_id, level, message, data)
            ...
            if (row.node_type === "condition") {
                ...
                const base = String(cfg.base ?? "context");
                if (!path) {
                    await client.query("rollback");
                    return await failWithRetryPolicy({
                ...
                set status = 'SUCCEEDED'::node_run_status,
                    ...
                    output = $3::jsonb,
                    updated_at = now()
              where id = $1`, [nodeRunId, ok ? "true" : "false", JSON.stringify(output)]);
                await client.query(`insert into node_run_logs (tenant_id, node_run_id, level, message, data)
             values ($1, $2, 'info', 'condition evaluated', $3::jsonb)`, [tenantId, nodeRunId, JSON.stringify(output)]);
                await client.query("commit");
                await engineAdvanceQueue.add("advance", { node_run_id: nodeRunId }, {
                    jobId: `advance-${nodeRunId}`,
                    removeOnComplete: 1000,
    ...
    nodeExecuteWorker.on("failed", (job, err) => {
