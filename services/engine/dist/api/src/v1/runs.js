...
export async function registerRunsRoutes(app) {
    const db = app.db;
    const runRequestsQueue = app.queues?.runRequests;
    // List runs (most recent first)
    app.get("/", async (req, reply) => {
        try {
            const tenantId = getTenantIdFromHeaders(req.headers);
            const r = await db.pool.query(`select id, tenant_id, workflow_version_id, trigger_event_id, status,
    ...
    app.get("/:runId/node-runs", async (req, reply) => {
        try {
            const tenantId = getTenantIdFromHeaders(req.headers);
            const runId = req.params.runId;
            const r = await db.pool.query(`select nr.id, nr.tenant_id, nr.workflow_run_id, nr.node_id, nr.node_type,
          ...
          order by nr.created_at asc`, [tenantId, runId]);
            return { items: r.rows };
        }
        catch (e) {
            const { statusCode, body } = toHttpError(e, app.log);
    ...
    app.get("/node-runs/:nodeRunId/logs", async (req, reply) => {
        try {
            ...
            const r = await db.pool.query(`select id, tenant_id, node_run_id, ts, level, message, data
           from node_run_logs
          where tenant_id = $1 and node_run_id = $2
          order by ts asc
          limit 500`, [tenantId, nodeRunId]);
            return { items: r.rows };
        }
        catch (e) {
            const { statusCode, body } = toHttpError(e, app.log);
    ...
    // ----
    app.post("/:runId/start", { config: { requiredRole: "editor" } }, async (req, reply) => {
        try {
            const tenantId = getTenantIdFromHeaders(req.headers);
            const runId = req.params.runId;
            ...
            const r = await db.pool.query(`update workflow_runs
          ...
          returning id, tenant_id, workflow_version_id, trigger_event_id, status,
                    started_at, finished_at, created_at, updated_at`, [tenantId, runId]);
            if (r.rowCount === 0) {
                // If it exists but is already RUNNING/terminal, treat it as idempotent.
                const cur = await db.pool.query(`select id, tenant_id, workflow_version_id, trigger_event_id, status,
                  started_at, finished_at, created_at, updated_at
             from workflow_runs
            where tenant_id = $1 and id = $2`, [tenantId, runId]);
                if (cur.rowCount === 0)
                ...
                if (cur.rows[0].status === "RUNNING") {
                    if (runRequestsQueue) {
                        try {
                            await runRequestsQueue.add("run_request", { run_id: runId }, {
                            ...
                            app.log.error({ err }, "failed to enqueue run_request for existing RUNNING run");
    ...
    app.post("/:runId/pause", { config: { requiredRole: "editor" } }, async (req, reply) => {
        try {
            const tenantId = getTenantIdFromHeaders(req.headers);
            const runId = req.params.runId;
            const parsed = UpdateRunStatusBody.safeParse(req.body ?? {});
    ...
    app.post("/:runId/resume", { config: { requiredRole: "editor" } }, async (req, reply) => {
        try {
            const tenantId = getTenantIdFromHeaders(req.headers);
            const runId = req.params.runId;
            const parsed = UpdateRunStatusBody.safeParse(req.body ?? {});
    ...
    app.post("/:runId/cancel", { config: { requiredRole: "editor" } }, async (req, reply) => {
        try {
            const tenantId = getTenantIdFromHeaders(req.headers);
            const runId = req.params.runId;
            const parsed = UpdateRunStatusBody.safeParse(req.body ?? {});
