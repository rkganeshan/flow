 ...
 */
export async function registerTriggersRoutes(app) {
    const db = app.db;
    app.post("/webhooks/:workflowId/:secret", async (req, reply) => {
        try {
            ...
            const wf = await db.pool.query(`select id, tenant_id, draft_graph
           from workflows
          where id = $1`, [workflowId]);
            if (wf.rowCount === 0)
                return reply.code(404).send({ error: "not_found" });
            const tenantId = wf.rows[0].tenant_id;
            const draftGraph = wf.rows[0].draft_graph;
            ...
            const ver = await db.pool.query(`select id, version
          ...
          limit 1`, [tenantId, workflowId]);
            ...
            const workflowVersionId = ver.rows[0].id;
            ...
            const dedupeKey = req.headers["x-flow-dedupe-key"] ?? req.id;
            const client = await db.pool.connect();
            try {
                await client.query("begin");
                const te = await client.query(`insert into trigger_events (tenant_id, source, dedupe_key, payload)
           values ($1, 'webhook', $2, $3::jsonb)
           on conflict (tenant_id, source, dedupe_key)
           do update set payload = trigger_events.payload
           returning id`, [tenantId, dedupeKey, JSON.stringify(parsedBody.data ?? {})]);
                const triggerEventId = te.rows[0].id;
                // 4) Create workflow run (idempotent via UNIQUE constraint).
                const run = await client.query(`insert into workflow_runs (
